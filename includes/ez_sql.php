	...
	*/
	...
	class ezSQL_mysqli extends ezSQLcore
	{
		...
		*/
		...
		// This function properly escapes values in an SQL query
		  ...
		  if (func_num_args() > 0)
		  {
			...
			foreach ($args as $arg) {
		  	...
		  	$query = call_user_func_array('sprintf',$args);
                  ...
                  //mail('matthew@webspacemail.com', 'safe query', $query);
		  if (substr_count($query, 'EXECUTE ') > 0) {
                      $query = substr($query, 8, strlen($query)-8);
                      //echo "Query: $query";
                      //die();
